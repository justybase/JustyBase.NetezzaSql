...
| 2.2 | Shared string-incremental `DatabaseTypeChooser` + contract | ✅ Done — engine+contract+unit tests. `ImportTypeAnalyzer` + `ImportColumnKind`/`DetectedImportColumnType` in `JustyBase.ImportExport.Import`; the host `DatabaseTypeChooser` maps the shared scan result (`ApplyScan`) and its `ExcelTypeDetection` is a thin adapter over `TabularImportScanner.ScanSource`. Golden-matrix parity test in the host (`ImportFromExcelFile_TypeInference_MatchesSharedGoldenMatrix`). |
| 2.3 | Move `DbImportJob`, `DBReaderWithMessages`, `ImportUsingOptions` into shared | 🚧 Partly done — `ImportUsingOptions`, `ImportValidationError`, `ImportColumnKind`, `ImportJob`/`IImportJob`, `ImportStepHelper` are shared (D0). `DbImportJob` deliberately stays a host adapter: it is the UI type-model (`DbTypeWithSize` chooser) wrapper over `IImportJob`; `DBReaderWithMessages` still host-side (used by `DataReaderFromExcelReaderAbstract`, which now takes shared kinds+headers). |
| 2.4 | Formalize Netezza import pipeline (pipe + DDL + `NetezzaImportHelper`) | ✅ Done — `NetezzaExternalTableImportEngine` (`JustyBase.ImportExport`) runs the CREATE random-distribution table + named-pipe EXTERNAL USING insert (ambient `ImportUsingOptions`, `BoolStyle 1_0`, nzbad detection) behind `IImportEngine`/`ImportEngineOptions`; the Avalonia plugin delegates and the host `NetezzaImportHelper`/helper project are removed. USING clause is a single SoT (`NetezzaImportSql.BuildUsingClause`) guarded by `NetezzaImportUsingClauseTests`. |
| 3.x | Shared import orchestrator + `IDbFileImportEngine` abstraction + clipboard seam | 🚧 Partly done — `TabularImportScanner` + `IImportSource`/`IImportSourceFactory` (D1) own inspect/scan/validate/create-jobs; `IImportEngine`/`BatchInsertEngine`/`NetezzaExternalTableImportEngine` (D2) execute jobs. Remaining: shared `IImportUseCase`/`IResultExportUseCase` contracts (4.x/5.x) and the clipboard seam, plus the generic batch engine wiring. |
