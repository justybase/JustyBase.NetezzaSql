...
public sealed class ParquetExportWriter(IDataReader rdr, int groupSize = 32_768)
{
    ...
    public async Task CreateFile(Stream fileStream)
    {
        ...
        await using (ParquetWriter parquetWriter = await ParquetWriter.CreateAsync(schema, fileStream, options))
        {
            int smallCounter = 0;
            while (_rdr.Read())
            {
                ...
                {
                    await WriteGroup(parquetWriter, smallCounter);
                    smallCounter = 0;
                }
            }
            if (smallCounter > 0)
        ...
        async Task WriteGroup(ParquetWriter parquetWriter, int smallCounter)
        {
            using ParquetRowGroupWriter groupWriter = parquetWriter.CreateRowGroup();
            for (int i = 0; i < filedsCount; i++)
            {
                await columns[i].WriteToGroupAsync(groupWriter, dataFields[i], smallCounter);
            }
        }
    }
}
...
internal sealed class OneColumn
{
    ...
    public async Task WriteToGroupAsync(ParquetRowGroupWriter writer, DataField field, int count)
    {
        if (_columnType == typeof(int))
        {
            await writer.WriteAsync(field, new ReadOnlyMemory<int?>(intValues, 0, count));
        }
        else if (_columnType == typeof(long))
        {
            await writer.WriteAsync(field, new ReadOnlyMemory<long?>(longValues, 0, count));
        }
        else if (_columnType == typeof(decimal))
        {
            await writer.WriteAsync(field, new ReadOnlyMemory<decimal?>(decimalValues, 0, count));
        }
        else if (_columnType == typeof(float))
        {
            await writer.WriteAsync(field, new ReadOnlyMemory<float?>(floatValues, 0, count));
        }
        else if (_columnType == typeof(double))
        {
            await writer.WriteAsync(field, new ReadOnlyMemory<double?>(doubleValues, 0, count));
        }
        else if (_columnType == typeof(DateTime))
        {
            await writer.WriteAsync(field, new ReadOnlyMemory<DateTime?>(dateTimeValues, 0, count));
        }
        else if (_columnType == typeof(bool))
        {
            await writer.WriteAsync(field, new ReadOnlyMemory<bool?>(boolValues, 0, count));
        }
        else
        {
            string?[] sliced = stringValues!;
            if (sliced.Length != count)
            {
                sliced = new string?[count];
                Array.Copy(stringValues!, sliced, count);
            }
            await writer.WriteAsync(field, sliced);
        }
    }
